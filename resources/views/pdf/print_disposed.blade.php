    ...
    <table>
        <tr class="with-border">
            ...
            <td style="border-bottom: none !important;" colspan="2" class="">
                Date: <span class="">{{ $diposedItemData->created_at }}</span>
            </td>
        </tr>
        <tr class="with-border">
            <td style="border-bottom: none !important;" colspan="2" class="">
                RRSP No.: <span class="">{{ $diposedItemData->rrsp_number }}</span>
            </td>
        </tr>
        ...
        @foreach($diposedItemData->items as $detail)
            <tr class="text-center with-border">
                @php
                    $returnedBy = $detail->returnedBy->firstname . ' ' . $detail->returnedBy->lastname ?? '' . ' ' . $detail->returnedBy->lastname;
                @endphp
                <td>{{ $detail->inventoryItem->item_desc ?? '' }}</td>
                <td>{{ $detail->quantity ?? 0 }}</td>
                <td>{{ $diposedItemData->ics_number ?? 0 }}</td>
                <td>{{ $returnedBy }}</td>
