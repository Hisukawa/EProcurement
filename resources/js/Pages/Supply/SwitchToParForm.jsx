...
export default function SwitchToParForm({ par, user, ppeOptions, type }) {
  const { toast } = useToast();
  const [showConfirm, setShowConfirm] = useState(false);
  const detail = par.po.details;
  const focal =
    `${par?.po?.details[0]?.pr_detail?.purchase_request?.focal_person?.firstname ?? ""} ${par?.po?.details[0]?.pr_detail?.purchase_request?.focal_person?.middlename ?? ""} ${par?.po?.details[0]?.pr_detail?.purchase_request?.focal_person?.lastname ?? ""}`
      .trim() || "N/A";
  const designation = par?.requested_by?.division?.division ?? "N/A";

  const { data, setData, post, processing, errors } = useForm({
    id: par?.id ?? "",
    type: type || "par",
    po_id: par?.po?.id ?? null,
    par_number: par.po?.po_number ?? "",
    requested_by: par?.requested_by?.id ?? user.id,
  ...
  // ✅ Function to fetch and generate property number (PAR)
  const updatePropertyNumber = async (index) => {
    const item = data.items[index];
    const { ppe, gl, office, school } = item;
    ...
    try {
      // ✅ Cache initialization (shared across items)
      if (!window.__parGlobalSeriesCache) window.__parGlobalSeriesCache = null;

      let baseSeries = 1;

      // ✅ Fetch once (latest from PAR/ICS)
      if (!window.__parGlobalSeriesCache) {
        const res = await fetch(`/api/par-next-series`);
        const response = await res.json();
        baseSeries = response.series || 1;
        window.__parGlobalSeriesCache = baseSeries;
      } else {
        // Increment globally for next item
        window.__parGlobalSeriesCache++;
        baseSeries = window.__parGlobalSeriesCache;
      }
      ...
      toast({
        title: "⚠️ Warning",
        description: "Failed to fetch next series number for PAR item.",
        variant: "destructive",
      });
    }
  };
  ...
  const confirmSubmit = () => {
    post(route("supply_officer.switch_to_par"), {
      preserveScroll: true,
      onSuccess: () => {
        toast({
          title: "✅ Success",
          description: "PAR issuance submitted successfully!",
      ...
      onError: () => {
        toast({
          title: "❌ Error",
          description: "Failed to submit PAR issuance.",
          variant: "destructive",
        });
        setShowConfirm(false);
      },
  ...
  return (
    <div className="bg-green-50 p-8 rounded-xl shadow-md">
      <h2 className="text-3xl font-bold text-green-800 mb-1 flex items-center gap-2">
        <FileText size={24} /> Property Acknowledgement Receipt (PAR)
      </h2>
      <p className="text-sm text-gray-700 mb-6">
        <strong>Note:</strong> Each item below must have its own PPE, GL, and
        location details before submission.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {data.items.map((item, index) => {
          const itemDesc = par.items[index]?.inventory_item?.item_desc ?? "N/A";

          return (
            <div
              key={index}
              className="border border-gray-300 bg-white rounded-lg p-6 shadow-sm"
            >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={item.ppe?.id || ""}
                      onChange={(e) => {
                        const selected = ppeOptions.find(
                  ...
                  <button
                    type="button"
                    onClick={() => updatePropertyNumber(index)}
        ...
        <div className="flex justify-end mt-6">
          <button
            type="submit"
            disabled={processing}
            className="inline-flex items-center px-6 py-2 bg-green-700 text-white rounded-md hover:bg-green-800 disabled:opacity-50"
          >
            <SendHorizonal size={16} className="mr-2" />
            {processing ? "Submitting..." : "Submit PAR"}
          </button>
        </div>
      </form>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white p-6 rounded-lg shadow-md max-w-md w-full">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to submit this PAR issuance?
            </p>
