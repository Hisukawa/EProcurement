...
export default function TwgUserLayout({ header, children }) {
...
const markAsRead = async (notification) => {
  try {
    ...
    if (notification.data?.pr_id) {
      Inertia.visit(route("twg_user.show_details", notification.data.pr_id));
    }
  } catch (error) {
      ...
      <aside className={`${sidebarOpen ? 'block' : 'hidden'} md:block md:w-64 w-full bg-gray-900 text-white md:sticky md:top-0 h-screen sticky z-50`}>
        <div className="p-6 flex flex-col h-full">
          ...
          <nav className="flex flex-col gap-2">
            <NavLink
              href={route('twg_user.dashboard')}
              active={route().current('twg_user.dashboard')}
            ...
            <NavLink
              href={route('twg_user.for_review')}
              active={route().current('twg_user.for_review')}
              className="flex items-center gap-2 px-4 py-2 hover:bg-indigo-600 rounded-lg"
            >
